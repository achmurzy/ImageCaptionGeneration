...
if read:
    ...
    encoder, decoder = dp.save_full_coco_lexicon(inputImgCount)
    ...
    encoderF = open(fileApp + "/encoder", 'w')
    pickle.dump(encoder, encoderF)
    encoderF.close()
    
    decoderF = open(fileApp + "/decoder", 'w')
    pickle.dump(decoder, decoderF)
    decoderF.close()
